...
{
	static TCHAR szAppName[] = TEXT("SysMets5");
	MSG msg;
	WNDCLASS wndclass;
	...
	auto hwnd = CreateWindow(szAppName, TEXT("Get System Metrics No. 5"),
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	static int iDeltaPerLine, iAccDelta = 0;
	...
	case WM_SETTINGCHANGE:
	{
		...
		if (scrollLines)
		{
			iDeltaPerLine = WHEEL_DELTA / scrollLines;
		}
		else
		{
			iDeltaPerLine = 0;
	...
	case WM_MOUSEWHEEL:
	{
		...
		iAccDelta += static_cast<short>(HIWORD(wParam));

		while (iAccDelta >= iDeltaPerLine)
		{
			SendMessage(hwnd, WM_VSCROLL, SB_LINEUP, 0);
			iAccDelta -= iDeltaPerLine;
		}
