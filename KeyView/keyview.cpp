...
INT WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR cmdLine, int iCmdShow)
{
	...
	auto hwnd = CreateWindow(szAppName, TEXT("Keyboard Message View No. 1"),
		WS_OVERLAPPEDWINDOW,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_SIZE:
	{
		...
		SetRect(&rect, 0, cyChar, cxClient, cyChar * (cyClient / cyChar));
	...
	case WM_SYSDEADCHAR:
	{
		...
		ScrollWindow(hwnd, 0, -cyChar, &rect, &rect); // newer on top change "-cyChar" to "cyChar"
